...
- [x] 顶栏副标题补思考等级显示（2026-09-06）— ChatHeaderTitle 副标题改为分段拼接：模型 · 上下文用量 · 思考等级（`info.reasoning_effort`，与 model 同源同刷新）。⚠ 踩坑：服务端口径 `""`=未设置（供应商默认，**不展示**，展示会让 desktop 的 sticky 选择被空值回写问题复现）、`"none"`=明确关闭（显示「思考关」）、其余为等级原文
