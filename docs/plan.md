...
- 多 profile：一个 serve 进程服务全部 profile；RPC 用 `params.profile` 作用域。本机实测 profile：default / main(示例 agent) / finance / mental-health / study / work。
...
- **M5**：定时任务管理三端实现（2026-09-10，§1 v3）。代码完成：`rpc/cron.ts`（REST 封装）+ `store/cron.ts`（cron.changed 自接线）+ `utils/cronSchedule.ts`（六模式构建/回显/中文描述）+ ViewSwitcher/CronPanel/CronRunsPanel/CronJobForm（手机/桌面共用面板）+ 手机 CronRuns/CronEdit 路由 + 桌面壳首屏切换与 Modal。jest 294 全绿（新增 cronSchedule 14 + cronStore 7）、tsc 0 错；web 冒烟（vite 代理真连 live dashboard，只读）：视图切换/列表徽章/行菜单/运行历史/新建表单/模式切换全部 PASS。真机验证交 构建机 出包。

## 7. 风险

- 原生模块无法本地编译 → 代码严格保守（经典 NativeModule 而非 TurboModule，减少 codegen 构建风险），构建机 首次构建时修。
