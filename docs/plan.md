...
- **M6**：界面多语言（i18n）—— **完成（2026-09-13，D047）**，框架契约见 `docs/i18n.md`。自研 `src/i18n/`（每语言单文件词典：`en.ts` 键权威源 + `zh-CN.ts` 类型强制对齐、`t()`/`useT()` 取用层、系统语言检测 + `hermes.locale.v1` 持久化覆盖、不支持的语言回退 en）；存量约 70 个文件 400+ 条硬编码中文文案全量迁移；`LanguageToggle` 三态循环切换（跟随系统/中文/EN；挂连接主页 + Profile 选择首屏，会话/聊天视图不提供——2026-09-14 按用户反馈从 rail 底部迁出）；`scripts/i18n-check.js` 守护（键对齐 + 硬编码 CJK 扫描 + 未使用键）入 CI 第 4 步；desktop 主进程独立翻译域（`desktop/i18n.ts` + `dt()`）。jest 328 全绿（新增 i18n 14 + 全局钉 zh-CN setup）、tsc/lint 0 错、`web-i18n-smoke.js` 四场景（英文系统/中文系统/覆盖优先/非法值回退）全 PASS；2026-09-14 用户 macOS 实测修复三问题（漏迁移的 JSX 文案「退出连接」、窗口标题卡死 Edit profile、切换入口位置）并新增 `desktop-i18n-smoke.js` 七场景回归，扫描器同步升级覆盖 JSX 文本。真机验证交构建机出包。
