...
- 断线恢复：服务端对孤儿会话有 ~20s 宽限（`HERMES_TUI_WS_ORPHAN_REAP_GRACE_S`），WS 重连后 `session.resume` 可重挂 inflight turn。服务端回收会话时广播 `session.reclaimed`。
  - **live sid 是易逝的**（2026-09-16 定因补充，server.py `_RECLAIM_END_REASONS`）：WS 断开超宽限（`ws_orphan_reap`）、闲置 TTL（`idle_timeout`，同样只在 transport 已死时生效）、LRU 上限（`lru_evict`）或网关进程退出都会使 live sid 失效，此后 `prompt.submit` 等打旧 sid 一律 4007。`session.resume`（持久化 id）快路径复用同一 live sid，**冷路径（原会话已被回收）返回新 live sid**——客户端必须以 resume 返回值为准迁移状态（App 的 bySession key / prompt.submit 都用 live sid）。
  - **`session.reclaimed` 是全局广播**（`_announce_session_reclaimed` → `_broadcast_global_event`）：帧级 `session_id` 为空串、向所有 transport 扇出，**身份在 payload 里** `{session_id: <live sid>, stored_session_id, reason}`——按帧级 session_id 路由事件的客户端会把它整个丢掉（App 曾因此收不到回收通知，2026-09-16 修复；wireEvents 需对该事件单独按 payload 接线）。
  - **空会话落库时机**：`session.create` 只建内存会话，**首次 `prompt.submit` 才写 sessions 行**（methods_prompt.py `_ensure_session_db_row`）——新建后未发过消息就被回收的会话：`session.resume` 必 4007、`session.list` 永远不可见（「新会话等待后发送报 session not found 且无法重进」的根因，客户端唯一出路是发送时重建会话）。
...
- 思考等级（2026-09-15 从源码核实 + App 落地，D049）：
  - `config.set {key:"reasoning", value:"<level>", session_id, scope?}`（server.py:14922）：等级集合 `none|minimal|low|medium|high|xhigh|max|ultra`（hermes_constants `VALID_REASONING_EFFORTS` + none，`parse_reasoning_effort` 校验，非法 → err 4002）。**会话级（默认）写 `session["create_reasoning_override"]`**；`scope:"global"` 改写 config.yaml `agent.reasoning_effort` 并清会话 override。**有 live agent 时两种 scope 都会** `agent.reasoning_config = parsed` + 持久化 runtime + **推 `session.info` 事件**；agent 未建（还没跑过 turn）则**不推事件**（`_emit` 在 `agent is not None` 分支内）——客户端要即时反映须成功后本地合并兜底。value 为 `show|hide|full|clamp` 时是显示开关（写 config.yaml display 段，不动等级、不推事件）。⚠ 注意：`session` 查无此 sid 时会**静默落到全局写 config.yaml**（`if global_scope or session is None` 分支）。
  - `config.get {key:"reasoning", session_id}`（methods_config.py:253）→ `{value, display:"show"|"hide"}`。回落链：会话 `create_reasoning_override` → live agent `reasoning_config` → config.yaml `agent.reasoning_effort` → **"medium"**（无显式设置时返回 "medium"，不会返回 ""——与 `_session_info` 事件的 `reasoning_effort:""`=未设置口径不同）。
  - ⚠️ **`slash.exec` 的 `/reasoning <level>` 对 live 会话无效**（App 顶栏「/reasoning 后不同步」bug 的根因，2026-09-15）：`reasoning` 不在 `_LIVE_SESSION_DIRECT_COMMANDS`（server.py:16446），被交给 **slash worker 子进程**执行；worker 里 `_handle_reasoning_command`（cli_commands_mixin.py:3788）只改子进程自己的 `reasoning_config`/`agent=None`——gateway 的 live 会话不受影响，`_mirror_slash_side_effects` 无 reasoning 分支 → 不推任何事件，且只有 `--global` 才写 config.yaml。**worker 不落任何可读回状态，「/title 式事后读回 config.get」救不了**（读回的是旧值）。→ App 客户端拦截 `/reasoning <level>`（含 `--global`/`--session`）改走上述 `config.set`（官方 desktop `model-menu-panel.tsx` 同款路径）；裸 `/reasoning` 查询与 show/hide/full/clamp 仍走 slash 流水线。
...
- ⚠️ **事件单播**：带 session_id 的事件帧只发给会话**最后绑定的 transport**（`write_json` → `session["transport"]`；create/resume/prompt.submit 都会 re-bind）。同一会话被 dashboard/TUI 打开过后，手机端就收不到审批/澄清事件。兜底：**resume 返回 `pending_approval`/`pending_clarify` 字段**（App 已在 attach/reattachAfterResume 消费恢复卡片），另有 `approval.pending {session_id}` RPC 可主动拉取；事件帧带 per-session `seq`，可用 `session.events.since {session_id, last_seen}` 补漏。
  ...
  - **clarify 全生命周期（2026-09-16 只读探查 server.py 确认，D052）**：clarify 是 turn 内的**工具调用**，事件序 = `tool.start{name:"clarify"}` → `clarify.request` →（挂起，**不发 message.complete**——turn 阻塞在工具回调里，续写文本按服务端口径属于同一条 assistant 消息）→ `clarify.respond`×N（全题锁定）→ `tool.complete{name:"clarify", result}` → 续写 `message.delta` → `message.complete`。要点：
    - clarify 的 tool.start/tool.complete **恒发**（`_tool_lifecycle_required_for_ui` 把 clarify/setup_mcp 列为「交互 UI 非可选装饰」，tool_progress off 也不抑制）。
    - `tool.complete.result` 是**答案回显**：单问=答案纯文本；批量=JSON `{"answers": {qid: answer}, "timed_out"?}`（`_block` 批量读出；超时时 locked 答案保留、`timed_out:true`）——客户端可据此收敛卡片（其他端作答场景，App 实现：`applyClarifyEcho`）。
    - 「整体提交」与协议零冲突：respond 本就逐题，客户端一次提交 = 按题目顺序连发 N 次（App D052 交互：草稿 + 卡片底部单按钮整体提交）。
    - **历史投影不含答案**：clarify 问答不作为独立历史消息，但 clarify **工具行**会进 `session.history`/resume 投影——`{role:"tool", name:"clarify", context, args}`，args 带问题（单问 `{question, choices, multi_select?}`；批量 `{questions:[{question, choices?, multi_select?}]}`，**无 qid**——qid 是运行期 wire id）；`_history_to_messages` 对 tool 行**只投影 args 不投影 result**，用户答案历史不可得。App 据此把历史 clarify 工具行合成只读已答卡（`hydrate`），位置=行位置。
...
- 会话/生命周期：`gateway.ready`、`session.info`、`session.usage`、`session.reclaimed`（全局广播，payload 带身份，见 §1 断线恢复）、`session.seeded`、`session.title {session_id, title}`（首轮后服务端自动命名时推送，**实测出现**）、全局 `sessions.changed`、全局 `cron.changed`（payload 空 `{}`；dashboard 进程每秒轮询 `~/.hermes/cron/jobs.json` mtime，变化即广播——App 用于定时任务列表自动刷新，见 §7）。
