...
- **文档**：README 提供 [English](README.md) 与 **简体中文** 两个版本；`docs/` 下的设计文档目前仅有中文。
- **App 界面**：支持**简体中文**与**英文**。界面**自动跟随系统语言**（不支持的语言回退英文），应用内提供小型切换入口（跟随系统 / 中文 / EN）。新增一种语言只需补一个词典文件，框架契约见 `docs/i18n.md`。
...
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | 贡献指南（分支规范、DCO、敏感信息纪律） |
| [`SECURITY.md`](SECURITY.md) | 安全策略与私密漏洞报告通道 |

---

## 许可

本项目采用 **[GNU Affero General Public License v3.0 或更高版本](LICENSE)（AGPL-3.0-or-later）** —— 一个 OSI 认可的自由软件许可证。

Copyright (c) 2026 CobyLee66

**你可以自由地**：使用、修改、分发本软件，**包括商业使用** —— 不需要付费，也不需要向作者申请授权。

**前提是**：如果你**分发**本软件或其衍生作品，或者把它**作为网络服务提供给他人使用**（AGPL 第 13 条），你必须以同样的 AGPL 许可公开**完整的对应源码**。

一句话概括：**自己用、随便改、拿去商用都行；但把闭源的衍生版本拿去分发或对外提供服务不行。**

### 为什么是 AGPL 而不是 GPL

本项目有浏览器/桌面形态，存在被改造成网络服务对外提供的可能。GPL 只约束「分发」行为，管不到「只在服务器上跑、不分发」的场景；AGPL 第 13 条补上了这个口子，能确保任何对外提供服务的修改版同样回馈源码。

### 你可以怎么用

| 场景 | 是否允许 |
|---|---|
| 自己安装使用（含在公司日常工作中使用） | ✅ 允许 |
| 修改后自己内部使用、不分发 | ✅ 允许 |
| 集成进商业产品并对外分发 | ✅ 允许，**但必须按 AGPL 开源你的衍生作品** |
| 改造成网站/服务对外提供 | ✅ 允许，**但必须向使用者提供完整源码** |
| 做成闭源产品出售 | ❌ 不允许 |
| 闭源地作为云服务对外运营 | ❌ 不允许 |

完整条款以 [`LICENSE`](LICENSE) 为准（该文件为 FSF 官方原文，未做任何改动）。

### 第三方组件

本项目依赖的库均为宽松许可（44 个 MIT、2 个 Apache-2.0），与 AGPL 兼容，不产生许可冲突。界面图标来自 [Tabler Icons](https://github.com/tabler/tabler-icons)（MIT）。

### 参与贡献

由于本项目采用 AGPL，贡献者提交的代码会以同样的许可发布。提交 PR 时请在提交信息中加 `Signed-off-by:` 行（[DCO](https://developercertificate.org/)），表示你有权提交该代码并同意以 AGPL 授权。详见 [`CONTRIBUTING.md`](CONTRIBUTING.md)。

### 安全

请勿用公开 Issue 报告安全问题，改用私密漏洞报告通道，详见 [`SECURITY.md`](SECURITY.md)。
